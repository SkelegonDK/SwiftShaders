# Diagnose and decide the fix for Motion Blur
...
User-reported (2026-08-26): the **Motion Blur** entry (`motionBlur`) is broken or its
example fails to show the effect. A true blur must sample neighbours (layerEffect); if implemented as a colorEffect it can only approximate — same structural class as chromaticAberration's triaged defect. Determine which defect class this is —
