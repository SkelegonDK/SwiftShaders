# Diagnose and decide the fix for Negative
...
User-reported (2026-08-26): the **Negative** entry (`negativeFilm`) is broken or its
example fails to show the effect. Compare against the separate Invert entry — if they render identically the entry demonstrates nothing distinct. Determine which defect class this is —
