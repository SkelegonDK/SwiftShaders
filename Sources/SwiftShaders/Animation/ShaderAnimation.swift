...
public final class ShaderAnimator: ObservableObject {
    ...
    #if os(macOS)
    // CADisplayLink's target/selector initialiser is unavailable on macOS,
    // so drive frames from a run-loop timer there instead.
    ...
    /// Tears down whichever frame source this platform uses.
