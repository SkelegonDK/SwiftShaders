...
static float sceneBasic(float3 p, float time) {
...
static float sceneMetaballs(float3 p, float time, float blobCount, float smoothness, float scale, float moveSpeed) {
    float d = 10000.0;
...
static float sceneSdfShapes(float3 p, float time, float operation, float smoothFactor, float shapeScale) {
...
static float tunnelCrossSection(float2 p, float shape, float radius) {
    int s = int(shape);
...
static float terrainHeight(float2 p, int octaves) {
    float h = 0.0;
    float amp = 0.5;
    float freq = 1.0;
