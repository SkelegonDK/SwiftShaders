...
static float voronoiF1(float2 uv, float scale, float time, float jitter) {
    float2 p = uv * scale;
    float2 ip = floor(p);
    float2 fp = fract(p);
...
static float2 voronoiF1F2(float2 uv, float scale, float time, float jitter) {
    float2 p = uv * scale;
    float2 ip = floor(p);
    float2 fp = fract(p);
...
static float3 voronoiWithID(float2 uv, float scale, float time, float jitter) {
    float2 p = uv * scale;
    float2 ip = floor(p);
    float2 fp = fract(p);
