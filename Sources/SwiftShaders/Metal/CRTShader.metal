...
static float2 barrelDistortion(float2 coord, float amount) {
    float2 cc = coord - 0.5;
...
static float scanline(float y, float resolution, float intensity) {
    return 1.0 - intensity * abs(sin(y * resolution * 3.14159));
}

// RGB phosphor pattern
static half3 phosphorMask(float2 uv, float scale) {
