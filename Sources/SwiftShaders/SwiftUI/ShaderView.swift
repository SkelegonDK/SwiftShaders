...
public struct ShaderStack<Content: View>: View {
    ...
    private let content: Content
    private let modifiers: [AnyShaderModifier]

    /// Creates a shader stack with multiple effects.
    public init(
        @ViewBuilder content: () -> Content,
        modifiers: [AnyShaderModifier]
    ) {
        self.content = content()
...
/// A type-erased `ViewModifier`, so stacks of differently-typed shader
/// modifiers can be stored in a single array.
///
/// Erasure happens in the generic initialiser, where the concrete type is
/// still known; an existential `any ViewModifier` cannot be applied directly.
@available(iOS 17.0, macOS 14.0, tvOS 17.0, visionOS 1.0, *)
public struct AnyShaderModifier: ViewModifier {
    private let apply: (AnyView) -> AnyView

    /// Wraps a concrete view modifier.
