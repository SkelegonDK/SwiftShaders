...
public struct FluidSimulationModifier: ViewModifier {
    ...
    public func body(content: Content) -> some View {
        content.colorEffect(
            ShaderLibrary.swiftShaders.fluidSimulation(
                .float(time),
                .float(viscosity),
                .float(turbulence),
                .float(flowSpeed),
...
public struct SmokeModifier: ViewModifier {
    ...
    public func body(content: Content) -> some View {
        content.colorEffect(
            ShaderLibrary.swiftShaders.smoke(
                .float(time),
                .float(density),
                .float(riseSpeed),
                .float(spread),
...
// MARK: - FluidWaterSurfaceModifier
...
/// ```swift
/// Image("background")
///     .modifier(FluidWaterSurfaceModifier(
///         time: animationTime,
///         waveHeight: 0.05,
///         waveFrequency: 3.0
///     ))
/// ```
@available(iOS 17.0, macOS 14.0, tvOS 17.0, visionOS 1.0, *)
public struct FluidWaterSurfaceModifier: ViewModifier {
...
public struct InkDiffusionModifier: ViewModifier {
    ...
    public func body(content: Content) -> some View {
        content.colorEffect(
            ShaderLibrary.swiftShaders.inkDiffusion(
                .float(time),
...
public struct PlasmaFluidModifier: ViewModifier {
    ...
    public func body(content: Content) -> some View {
        content.colorEffect(
            ShaderLibrary.swiftShaders.plasmaFluid(
                .float(time),
                .float(complexity),
                .float(colorSpeed),
                .float(scale),
...
public struct MagneticFieldModifier: ViewModifier {
    ...
    public func body(content: Content) -> some View {
        content.colorEffect(
            ShaderLibrary.swiftShaders.magneticField(
                .float(time),
...
public struct OilSlickModifier: ViewModifier {
    ...
    public func body(content: Content) -> some View {
        content.colorEffect(
            ShaderLibrary.swiftShaders.oilSlick(
                .float(time),
