...
#### ✅ 7a RESULTS — executed 2026-08-05 (per the ADV‑1 amendments in `plans/01`, which supersede the paragraph above)

**No descriptor was authored.** ADV‑1 rejected that direction: `ShaderBinding.Descriptor` already owns
the word, and generating 91 gallery entries from a library-side descriptor would have meant inventing
~350 slider ranges. What shipped instead is an **inventory with a ratchet** — the gallery stays
hand-curated, and tests enforce that it represents the library.

| | Before | After |
|---|---|---|
| Competing effect counts | 30 / 91 / 208 | **one ledger of 226**, 91 in the gallery |
| `ShaderCatalog` registry | 30 rows, 19 phantom | **deleted** |
| Public `View` effect methods | unknown | **226** (219 distinct names), scanned and asserted |
| Ambiguous overloads | `voronoiNoise` resolved silently | **0**, with a test that would have caught it |
| `swift test` | 80 | **84 tests, 0 failures** |

**What landed**

| File | What it is |
|---|---|
| `Core/ShaderCatalog.swift` | **deleted.** 19 of 30 ids named no public method, `gaussianBlur` no Metal function. Its 5 `View` methods moved next to their modifiers, signatures unchanged. |
| `Sources/SwiftShadersGalleryCore/` | **new library target** — `Effect`, `EffectCatalog`, `SampleElements`. The executable is now `@main` + `ContentView` + `Pasteboard`. Tests import the catalogue; the 233-line text parser (`Support/EffectCatalogSource.swift`) is deleted. |
| `Tests/…/Support/EffectCoverageLedger.swift` | **the deliverable.** All 226 methods: 91 `.inGallery`, 14 `.presetOf`, 121 `.deliberatelyAbsent` with one of four documented reasons. |
| `Tests/…/Support/PublicViewMethodScanner.swift` | Structural scan of `public extension View` blocks, paired with a line-shape raw count as an independent yardstick. |
| `Tests/…/EffectCoverageTests.swift` | 10 tests: no phantoms, completeness both ways, preset referential integrity, the ratchet, the scanner's own yardstick. |
| `Tests/…/GalleryRenderSweepTests.swift` | Renders all 91 entries; each must differ from the unshaded control and not be one flat colour. 4 tests, ~0.9 s. |
| `Tests/…/DuplicateEffectNameTests.swift` | The 4-name duplicate ratchet **plus** an ambiguity test over accepted call shapes. |
| `Tests/…/UnboundFunctionInventoryTests.swift` + `Resources/unbound-functions.txt` | The 49 unreachable stitchable functions, generated by `extract-metal-signatures.py`, with a staleness guard and a ratchet. |
| `Tests/…/ReadmeClaimsTests.swift` | The README's counts, and every effect its code fences call. |

**`voronoiNoise` → `cellularNoise`.** Both overloads accepted `(time:)` and `(time:scale:)`; Swift's
fewest-defaults tiebreaker silently picked `Noise`'s. The deprecated shim carries **no default values**,
which is what actually removes the ambiguity — a shim with defaults reproduces the bug, and the
negative control proves it (restore the defaults → red).

**🔴 Three real findings the sweep produced, none of them 7a's to fix:**

1. `voronoiShattered` and `voronoiStainedGlass` (`VoronoiShader.metal:263`, `:541`) declare `float time`
   and pass `0.0` to their voronoi calls — "static voronoi for consistent cracks". The gallery drives
   them with a `TimelineView` that redraws a still image every frame.
2. `infiniteGrid` (`RaymarchingShader.metal:464`) **never draws its grid.** Its antialiasing divides by
   `fwidth(p.xz / gridSize)`; the quotient always exceeds the smoothstep's upper edge, so
   `gridIntensity` is 0 everywhere and only the static horizon glow renders. The grid is the only
   time-dependent term, which is how the sweep found it.
3. The README's opening example called `.hologram()`, `.glitch()`, `.ripple()` and `.fire()` — **four
   methods that do not exist.** Fixed, and now enforced.

**🔴 The trap of this phase: the sweep's first version could not fail.** It compared renders
byte-for-byte over a four-stop `LinearGradient`. `ImageRenderer` dithers gradients slightly differently
depending on what was rendered before, and that noise (491, as a sum of absolute differences) was
*larger than several real effects* — so the negative control that replaced an entry's closure with the
identity stayed **green**, twice, and looked like a working test. The fix was to remove the noise rather
than threshold around it: a source of four flat quadrants and a disc dithers nothing, and the same
measurement drops to **2** while the weakest real effect scores 806. A test now asserts that noise
stays far below the threshold, so this cannot come back silently.

> **Generalise it.** Phase 6's lesson was *a guard that measures itself is not a guard*. This is its
> sibling: **a comparison is only a test if you have measured what the two sides differ by when nothing
> is wrong.** Neither is visible from a green run.

**Negative controls — 12, every one red, each edit and each restore confirmed against `git diff`:**
phantom gallery id · ledger omission · ratchet raised · scanner blinded · new duplicate name ·
ambiguity restored · entry forced black · entry made inert · animated entry frozen · inventory made
stale · README count falsified · slider default out of range.

**Deferred, deliberately:** the 121 missing gallery entries (the ratchet makes the backlog visible;
authoring them is ~350 hand-tuned slider ranges), and the two `animated: true` entries that ignore
their time argument, which need the Metal signature decision above.
